...
**Update (2026-07-08, first post-merge CI nightly):** 17/18 — `memory_carryover`'s recall probe ("what OS does my laptop run?") collided with the knowledge agent's ADR-017 grounding contract: with no KB article covering the question, the agent must refuse, and in CI it did — while using the injected fact in all four local runs (instruction-conflict resolution is genuinely nondeterministic). Probe-design bug, fixed in the probe: session B now asks a legitimate KB task the fact PERSONALIZES ("which VPN setup guide applies to my laptop?" — the KB ships separate macOS and Windows guides, the injected Windows fact must pick one), which is the product behavior long-term memory exists for. Verified 3/3 locally; the fact-row side-effect assertion is unchanged and the 1.0 floor untouched. Every other suite passed on that nightly (quality's best run yet: faithfulness 4.7, helpfulness 5.0 — fourth report-only observation).

---

## ADR-035: CMDB graph = one generalized `cis` node table + one `dependencies` edge table; teams are nodes; assets stay separate

Date: 2026-07-07 · Status: accepted

**Context:** M9 needs infrastructure topology — services, servers, databases and who-depends-on-what — for impact analysis ("db-server-01 is down, who cares?") and the Graph-RAG comparison. The existing `assets` table looks adjacent but isn't: assets are user-owned end devices (owner, OS, model — a laptop), while infrastructure CIs are shared (dependents, an owning org, no user). The modeling question: extend `assets`, one table per CI kind, or one generalized node table?

**Decision:** Two new tables (migration `0003`), queried only through the graph tool (ADR-004). (1) **`cis`**: one node table with a `ci_type` discriminator (`service/server/database/team`) — per-kind tables would force polymorphic edges (a `(kind, id)` pair with no real FK), whereas one table gives the edge table genuine FK integrity and keeps the recursive traversal a single self-join. `name` is unique and is the public identity: tools and eval ground truth address CIs by name ("auth-service"), never UUID — an LLM can copy a name from a user's sentence; it cannot know a UUID. (2) **`dependencies`**: directed edges reading *dependent DEPENDS ON dependency* (`runs_on/uses/calls` as display metadata), unique per pair, self-loops CHECKed out, both columns indexed because impact analysis traverses against the arrows and root-cause with them. Cycle safety is the traversal's job (path-array guard in the CTE), not the schema's — real CMDBs contain cycles (service meshes), so the schema must allow them. (3) **Teams are nodes** (`ci_type='team'`, `owner_org` → users-by-org rollup): "which teams/users are impacted" becomes the same traversal as everything else plus one COUNT, instead of a special-cased join bolted onto every query. (4) `assets` untouched: conflating end devices with infrastructure would force every asset query to filter out servers and vice versa — two kinds of thing, two tables.

**Alternatives:** Extending `assets` with nullable infra columns (rejected above); per-kind tables (no FK-sound edges, three-way UNION in every traversal); a `dep_type`-aware traversal contract (YAGNI — impact doesn't care whether the dependency is `runs_on` or `calls`; the type is kept as payload metadata only); modeling teams as a join table `team_service_usage` (a second edge namespace that every impact query would have to union in).

**Tradeoffs:** A generalized table means thin, nullable metadata columns (`owner_org` is NULL for servers/databases); acceptable — the graph is about edges, not node attributes. Teams-as-nodes slightly stretches "configuration item", flagged in the table comment; the payoff (one traversal answers the user-impact question) is the point of the milestone.

---

## ADR-036: Graph eval = three-way comparison with a deliberately fair plain-RAG arm, closed-universe name scoring, and tool-level latency

Date: 2026-07-07 · Status: accepted

**Context:** M9's flagship claim is "Graph-RAG beats plain RAG on multi-hop questions." A comparison where plain RAG has zero relevant text is a strawman — nothing in the KB mentioned the infrastructure, so plain RAG would score 0 by construction and the claim would be empty. The eval also has to score three arms the same way, and compare CTE vs Cypher somewhere the difference is actually visible.

**Decision:** `SUITES["graph"]` (nightly-only), 15 multi-hop cases (`evals/datasets/graph.jsonl`) with committed ground-truth sets, three arms over the SAME questions: (a) plain RAG = the knowledge agent, articles only; (b) CTE Graph-RAG = the incident agent + query_dependency_graph on postgres; (c) Neo4j Graph-RAG = the same via the config switch (self-skips, reported not failed, when Neo4j is absent — CI has none). Fairness design: (1) **seven runbook articles carry every one-hop fact in the graph** (each service's servers/databases/callers/teams + a database-hosting map), so plain RAG has all the information and lacks only the join; their bodies are **deterministic templates, not LLM prose** — they carry eval ground truth, and a garbled fact would rig the comparison. Multi-hop questions still require chaining runbooks by construction (each documents only its own services' one-hop neighborhood). (2) **Scoring is arm-blind**: extract CI names from the final answer (closed 59-name universe, word-boundary regex, "sales team"-style aliases for team nodes, citation titles/Sources blocks stripped first — runbook TITLES contain CI names), filter to the case's scope_types, exclude the queried entities, then precision/recall/F1 vs the committed set. Every question demands "list them by name". (3) **Ground truth committed, never recomputed**: expected sets were computed from the seeded graph at dataset-authoring time and hand-checked against the taxonomy plan — a traversal regression can't rewrite the answer key it is graded against. (4) **Latency compared at the TOOL level, LLM-free** (every case query × 20 reps per backend, p50/p95): agent latency is LLM-dominated (~10 s p50) and would drown a ~2 ms traversal. (5) **Backend parity asserted LLM-free** (identical (name, depth) sets per case) — a deterministic invariant, not a floor.

**Measured (2026-07-07, three same-day runs):** CTE and Neo4j arms: **F1 1.000, 15/15 exact sets, all three runs** — the tool returns the complete set and the agent relays it faithfully. Plain RAG: **F1 0.44 / 0.64 / (run 3 pending at authoring; final numbers in README)**, with 5 / 3 refusals — the ADR-017 grounding contract sometimes judges chained runbook evidence insufficient and refuses outright, so plain RAG's multi-hop score is not just lower but HIGH-VARIANCE (the same instruction-conflict nondeterminism ADR-032 documented). Tool latency: postgres ≈ neo4j ≈ 2–3 ms p50 at 59 nodes/82 edges — no traversal-speed story at this scale. Floor: `cte_f1_mean ≥ 0.8` (below a 1.000/1.000/1.000 baseline; red means the agent stopped reaching for the graph or garbles its output — a couple of flubbed relays stay green per ADR-026); the rag arm is deliberately floor-free (it is the comparison baseline, not a product contract), and the neo4j arm gates via parity, not a floor (it is absent in CI).

**Alternatives:** Scoring with an LLM judge (adds judge variance to an exactly checkable task — the expected sets are literal); giving the knowledge agent the graph tool and diffing with/without (breaks the ADR-017 citations contract the refusal eval keys on; the incident agent is where impact questions belong per ADR-035); agent-level latency comparison (drowned, above); a live-recomputed ground truth (silently self-consistent with bugs).

**Tradeoffs:** Name extraction under-credits an answer that says "all four teams" without naming them — accepted, the questions demand names and it penalizes every arm equally. The rag arm's score partly measures the refusal contract rather than pure retrieval — that is reported explicitly (refusal count per run) rather than smoothed over, because "the agent declines multi-hop assembly" IS a real behavior of RAG-only systems.

---

## ADR-037: Graph backends = one contract, Postgres CTE default, Neo4j as an optional synced projection that fails loudly

Date: 2026-07-07 · Status: accepted

**Context:** The original M9 ask was "Graph-RAG with Neo4j". Standing up Neo4j as the PRIMARY graph store would mean a second source of truth, a second seeding path, and a hard runtime dependency for a feature one tool uses — while the dependency data already lives in Postgres tables one recursive CTE away.

**Decision:** (1) **Both backends implement one traversal contract** (`graph/`): `query(ci_id, direction, max_depth) -> [{name, ci_type, owner_org, depth}]`, depth = shortest hop, start excluded, sorted — selected by `settings.graph_backend`, **default postgres**. The tool layer owns name→CI resolution (against Postgres — the source of truth regardless of backend) and the teams→user-count rollup, so backends stay pure traversal and the latency comparison stays apples-to-apples. (2) **Postgres is the system of record; Neo4j is a derived projection** kept in sync by an idempotent, convergent script (`python -m graph.sync_neo4j`: MERGE all nodes/edges, then delete anything Postgres no longer has). (3) **Neo4j absent ⇒ the postgres path is untouched and the neo4j path fails LOUDLY** — the driver import is lazy (never a prerequisite for postgres), `query()` propagates connection errors, and there is deliberately NO silent fallback: a fallback would make the config switch a lie and the eval comparison meaningless. Tests/evals may skip via `ping()`; the tool path never calls it. (4) Cypher parity detail: relationship isomorphism terminates cycles for free but RETURNS THE START NODE when a cycle closes back on it — the CTE's path guard never does; the synthetic-cycle test caught the divergence and the Cypher carries an explicit `WHERE n <> start`.

**The tradeoff writeup (measured):** the traversal query is 16 lines of SQL vs 5 of Cypher — Cypher genuinely wins the query-ergonomics point (`*1..10` IS the recursion). Everything around the query reverses the sign: the postgres backend is one 64-line module and zero new infrastructure; the Neo4j path is a driver dependency + an 81-line backend + a ~95-line sync script + a compose service + credentials + a staleness failure mode that exists the moment the projection does. At this graph's scale (59 nodes / 82 edges, ≤5 hops) traversal latency is identical (~2–3 ms p50 both, measured LLM-free) and answer quality is identical (parity-tested, F1 1.000 both arms). **When Neo4j would earn its cost:** unbounded/large depth (index-free adjacency keeps per-hop cost flat where the CTE's working-table joins grow), varied graph-shaped queries (shortest path, centrality, pattern matching — painful as CTEs), or when the graph is the product rather than one feature. None of those hold here, which is why postgres is the default and Neo4j is the demonstrably-swappable option — that swap being demonstrable is the portfolio point.

**Alternatives:** Neo4j-only (rejected above); pgRouting/AGE Postgres extensions (non-stock images — the compose gotcha and CI service containers get harder for no capability we need); implementing the interface as a class hierarchy (two functions behind a factory is the whole requirement).

---

## ADR-038: Slack ingestion over Socket Mode, with the runner as a pure HTTP client of the chat API

Date: 2026-07-07 · Status: accepted

**Context:** M8 connects Slack threads to the incident pipeline: on a trigger, the thread becomes a ticket (or a dedup link) and the reply lands back in the thread. Two Slack delivery models exist — the Events API (Slack POSTs to a public HTTPS endpoint) and Socket Mode (the app opens an outbound WebSocket). Separately, the ingestion process needs SOME path into the agent pipeline, and every extra path is a place where identity handling, session policy, or the semantic-cache gate can silently fork.

**Decision:** (1) **Socket Mode** (`app/slack/runner.py`, `make slack`): the deploy story is still in flux (ADR-009/029 — no live deployment exists), and Socket Mode needs no public URL, no tunnel, and no Request-URL verification — the entire flow runs from a laptop, which is where this project demos. The tradeoff is explicit: a persistent WebSocket to babysit and Slack's rate preference for it make the **Events API the right shape for an always-on production deployment**; the swap is kept cheap by concentrating ALL logic in `parse_trigger` + `handle_trigger` — only `main()` is Socket-Mode-shaped, and an Events-API FastAPI route would call the same two functions. (2) **The runner is a pure HTTP client of `POST /chat`** — exactly like the Streamlit UI. It never imports an agent, never touches the DB, and never calls a tool; the "normal pipeline" requirement is satisfied by construction because there IS no other pipeline entry. Slack-specific behavior rides in the request (`source="slack"`, thread coordinates, the untrusted-content envelope), and routes_chat gates the chat-only conveniences off it: no semantic-cache serve (a report that paraphrases a KB question must still become a ticket, not a cached answer) and no memory hooks (extracting "user facts" from multi-author thread text would poison the acting user's memory). (3) **Deterministic sessions**: session_id = `slack:{channel}:{thread_root}`, so a re-trigger on the same thread continues the same conversation and the agent sees its own earlier decision. (4) **Two triggers** — @mention and the `:ticket:` reaction — because the reaction is the natural gesture on someone ELSE's already-written report; the triggering user, not the thread author, is the acting user (they are the one requesting the filing, and their identity is the one verified).

**Alternatives:** Events API now (needs a public URL the project doesn't have; revisit at deployment — the seam is two functions); Bolt framework (`slack_bolt`) instead of the plain SDK (a listener-decorator abstraction over what is here ~40 lines of `main()`; the plain `slack_sdk` SocketModeClient keeps the moving parts visible); the runner importing `Runner.run` directly (a second pipeline entry that would have to re-implement session policy, cache gating, memory gating, and the guardrail contract — the exact fork this ADR exists to prevent); replying from the runner instead of the agent (the agent knows what it decided — created vs linked, which ticket, which article; the runner would have to parse that back out of prose).

**Tradeoffs:** A long-lived runner process is one more thing to keep alive locally (acceptable: it is optional, and everything else runs Slack-less). Fire-and-forget triggers mean the agent cannot ask the reporter follow-up questions — the incident instructions handle Slack reports as one-shot (file with what you have); measured in the baseline runs after one run ended on a clarifying question nobody could answer.

---

## ADR-039: One identity boundary for both external surfaces: verified externally, resolved internally, fail-closed

Date: 2026-07-07 · Status: accepted

**Context:** M8 adds the system's first two EXTERNAL identity surfaces: Slack users triggering ingestion and MCP clients calling tools. The internal invariant since M2 (user_tools DESIGN NOTE): tools read the acting user from `ChatContext` via the run context — never from an LLM argument. External surfaces must MAP their own identity artifacts (a Slack profile, a bearer token) onto that same boundary without creating a second path.

**Decision:** (1) **Slack → email match, fail-closed** (the runner): the triggering user's Slack profile email (`users.info`, `users:read.email` scope) must equal a seeded `users.email`, checked via `GET /identity/resolve` BEFORE the pipeline runs. No match → a canned in-thread apology from the runner and NOTHING runs — no guest identity, no "file it as the demo user", no ticket under someone else's name. The matched email goes into `ChatRequest.user_id` exactly as the Streamlit picker does; `resolve_acting_user` inside the tools stays the single place identity is trusted. (2) **MCP → static bearer-token map**: `MCP_TOKENS="token=email,…"` (env), verified per-request by the MCP SDK's own `BearerAuthBackend` against a `StaticTokenVerifier`; the verified token's email reaches tools as `ChatContext.user_id` through the SDK's auth contextvar. **Full multi-user auth — OAuth flows, token issuance, rotation, scopes — is deliberately out of scope**: the boundary being demonstrated is token→user→ownership-guards, not an identity product. (3) **Slack thread coordinates are trusted per-run state, not LLM arguments**: `post_slack_message` reads channel/thread from `ChatContext` (set by routes_chat from the runner request), so a prompt-injected model can compose reply TEXT but can never choose a destination — the same argument-trust layer as user_id, applied to "where output goes". (4) **The API trusts its clients** (unchanged M0 posture, now stated where it matters): `user_id`, `source`, and `slack_*` are honest-client fields — the API itself has no authentication layer, which is exactly the kind of thing ADR-009's deploy checklist owns. (5) **Slack-less by construction everywhere that gates**: `post_slack_message` no-ops into a logged error dict without credentials; the eval suite drives recorded thread fixtures through the REAL runner functions with a fixture gateway, and asserts replies via the `SLACK_SINK_FILE` seam (the spawned server writes would-be posts to a file) — no live workspace in CI, ever.

**Alternatives:** Auto-creating a users row for unmatched Slack users (an external actor writing to the identity table — inverts the trust direction); mapping unmatched users to a shared "slack-guest" account (tickets owned by nobody; ownership guards would then protect nothing); Slack user-id ↔ user-id mapping table (real ITSM products do this; email matching demonstrates the same boundary against the seeded data without a new table and a management UI); tokens in a DB table for MCP (rotation/audit belongs to the out-of-scope identity product; an env map is honest about being static demo auth); the agent posting the no-match apology (running the pipeline with no resolvable identity so an LLM can apologize is backwards — the refusal must not depend on a model).

**Tradeoffs:** Email matching makes the live Slack demo require a profile-email tweak (documented in SLACK_SETUP.md §3). The static token map cannot express scopes or expiry — accepted as the documented scope cut. The sink seam is test-shaped code in a production module; it reuses the no-op branch the Slack-less contract already requires, and it is what makes reply CONTENT assertable in CI.

---

## ADR-040: Agent tools vs MCP tools — one plain function layer, two thin adapters; the MCP surface is read/create only

Date: 2026-07-07 · Status: accepted

**Context:** M8 exposes ITSM capabilities over MCP. The tools already exist twice-shaped: plain functions (unit-testable, ADR-004's only-DB-path) and `@function_tool` wrappers for the Agents SDK. The design question the original prompt asks explicitly: what is the relationship between "tools an agent holds" and "tools an MCP server exposes"?

**Decision:** (1) **They are the same functions behind different adapters, and neither adapter contains logic.** The Agents SDK path wraps the plain layer for the in-process chat agents; `mcp_server/server.py` wraps the SAME plain functions with `@mcp.tool()` for external clients. Schemas derive from shared signatures/docstrings; every guard (identity, parse→exists→ownership, enum checks, price-from-DB) lives in the plain layer and therefore holds for BOTH callers plus any future third adapter. The distinction that matters: **an agent's tool list is a per-agent capability decision inside one orchestrated graph** (the router/specialist architecture decides who may do what, pinned in test_agents.py), while **the MCP tool list is a product surface for arbitrary external orchestrators** — a different trust posture, chosen independently. (2) **The MCP surface is deliberately read/create only**: search_knowledge_articles, list_catalog_items, create_ticket, get_ticket_status. No update_ticket (mutation of existing state from outside the conversational flow), no order placement or approval (ADR-005/020: approval authority is human-only; placement is wired to the HITL state machine the MCP client cannot see), no asset_id on create_ticket (get_user_assets isn't exposed, so a client has no legitimate way to obtain one). Pinned in tests/test_mcp.py the way agent tool sets are pinned. (3) **get_ticket_status is a new plain function** (house pattern: parse→exists→ownership) with deliberately NO agent wrapper — the chat agents read tickets through richer tools; it exists because an external client that created a ticket needs a way to poll it. Its ownership gate contrasts add_ticket_comment's deliberate foreign-ticket allowance: status READS of other users' tickets are an information leak, "me too" comments are the dedup feature. (4) **Transport: stateless streamable HTTP** on :8090 — every call carries its own verified Authorization header (per-request identity per ADR-039), no session affinity, works with Claude Desktop via `mcp-remote`. Verified end-to-end with a real MCP client (`ignore/tem/m8_mcp_smoke.py`: 401 on bad/missing token, exact tool list, create→row for the token's user→status read-back, foreign-ticket refusal through the wire).

**Alternatives:** The MCP SDK's Agents-SDK integration re-exporting the `*_tool` wrappers (couples the external surface to in-graph capability decisions — adding a tool to the incident agent must not silently publish it to the internet); exposing the agents themselves over MCP (e.g. a "file_ticket" prompt-level tool running the incident agent — hides the deterministic layer this project exists to showcase, and bills LLM calls for what is a function call); stdio transport (no bearer story — auth would become "whoever can spawn the process"; HTTP + header is the documented Claude Desktop remote pattern); exposing update_ticket for symmetry (mutation without the conversational ownership context; nothing needs it).

**Tradeoffs:** Four thin `@mcp.tool()` wrappers duplicate signatures/docstrings of the plain functions (accepted: the duplication IS the adapter, and schema drift shows up as a diff in one reviewable file). Static bearer + no OAuth means Claude Desktop connects through `mcp-remote` rather than natively as a first-class connector. `AuthSettings` requires nominal RFC 9728 issuer/resource URLs that serve no OAuth flow here — documented as ceremony the SDK demands when auth is on.

---

## ADR-041: Guardrails where untrusted input actually enters: a Slack-gated injection screen that flags-then-refiles, with tool-level checks as the real backstop

Date: 2026-07-07 · Status: accepted

**Context:** Guardrails were the old plan's M7, folded into M8 because Slack threads and MCP clients are the system's first UNTRUSTED input surfaces — until now every input came from the signed-in user the tools already act as. The threat that matters here is steering: thread text like "ignore previous instructions", "approve order X", "close ticket Y" trying to make the ingestion agent act beyond filing the report. Requirement: such text must be treated as REPORT CONTENT, not commands — and explicitly not silently dropped (a lost report is the incident pipeline's cardinal failure mode). Also owed: a measured decision on a general off-topic guardrail for the chat path.

**Decision:** Three layers, ordered by how much they are trusted. (1) **The tool-level identity/ownership checks (M2's "interim" defense) are the real backstop, now promoted to permanent security posture:** a fully steered model has nothing dangerous to call — no agent holds approve/reject tools (ADR-005), update_ticket/get_ticket_status enforce ownership, identity and the Slack reply destination come from the run context (ADR-039), and the MCP surface is read/create (ADR-040). The guardrail is defense-in-depth and an observability signal, never the load-bearing wall — instructions can be argued with; tool guards cannot. (2) **Deterministic framing + hardened instructions:** the runner wraps thread text in an untrusted-content envelope (quoted verbatim between explicit markers), and the incident agent's instructions draw the line the baseline runs proved necessary: embedded commands targeting the agent's role or OTHER entities are content to record; the reporter's ordinary wishes about THIS report ("add us to the existing ticket", "this is urgent") feed normal judgment — the first hardening draft banned "touching other tickets" wholesale and measurably suppressed legitimate dedup-linking (slack suite runs 1/3/4) until the line was drawn between commanded-by-text and found-by-your-own-search. (3) **An SDK input guardrail** (`slack_injection_guardrail` on the router — input guardrails only run on a run's first agent, and every routes_chat run starts there): gated to `source=="slack"` so the chat path pays zero latency and zero cost; `run_in_parallel=False` so the tripwire fires BEFORE any specialist tool call rather than racing it; classification by a small structured-output screen on the triage model. **On a trip, the report is refiled, not refused:** routes_chat returns `flagged=True` (a first-class response, not an HTTP error), and the runner re-submits ONCE with `injection_screened=True` (the guardrail's skip condition) plus a security preamble disclosing the finding — the ticket gets created with the malicious text preserved as evidence, the embedded commands are not obeyed, and the eval's trap case asserts all of it (flagged + ticket exists + every order's approval_state unchanged + the named open ticket still open + reply posted): 7/7 across every baseline run including pre-fix code. (4) **Off-topic guardrail on the chat path: measured and deliberately NOT wired.** Measurement (`ignore/tem/m8_offtopic_measurement.py`, output committed alongside): the candidate gpt-5-mini relevance screen classifies 6/6 probes correctly at p50 3.3 s and ~$0.0004/call, but the pipeline ALREADY lands every off-topic probe (3/3) on the ADR-017 refusal contract — a polite no with a ticket offer. Sequential wiring would tax EVERY legitimate turn ~3.3 s to make a rare off-topic answer fail faster; parallel wiring removes the latency but keeps the per-turn bill and adds a mid-run cancellation race. The refusal cascade is the off-topic guardrail; revisit only if a deployed surface shows real abuse volume.

**Alternatives:** Tripwire-as-rejection (blocks the report entirely — turns an attacker's embedded string into a denial-of-service on legitimate reports; the flag-then-refile design makes injection attempts *strictly less* effective than a plain report); guardrail on the incident agent (input guardrails don't run on handoff targets in this SDK — it would never fire); `run_in_parallel=True` for latency (the router's turn is short and the race window is exactly where a forced first tool call lives; Slack ingestion is async, seconds are free); deterministic regex/keyword screen (the strings worth catching are natural language; the committed fixture would pass a regex and the next phrasing wouldn't); sanitizing/stripping the malicious text before filing (the embedded command IS evidence — IT should see what was attempted, and silent mutation of report content is its own integrity failure).

**Tradeoffs:** Flagged threads cost one extra classifier call + one extra full run (paid only on flags; measured ~40 s total for the trap case). The screen itself is a model and will have false verdicts: a false positive costs one disclosed re-run (the report still lands), a false negative leaves layers 1–2 — which is survivable by design, and why the eval asserts end-state invariants rather than classifier verdicts. `injection_screened` is honored from the request (a lying client could skip the screen) — consistent with the ADR-039 trusted-client posture.

## ADR-042: Langfuse Cloud as the tracing backend, wired as an Agents SDK trace processor; empty keys are a verified no-op

Date: 2026-07-08 · Status: accepted

**Context:** M6 turns the last stub (`app/observability/tracing.py`) into the observability layer. Two decisions bundled here because they constrain each other: where Langfuse runs (Cloud vs a local docker-compose stack), and how traces get there (Langfuse's own OpenAI-SDK/OTel instrumentation vs a bridge from the Agents SDK's native tracing). Hard constraints: CI has no Langfuse secrets and must stay green (the e2e/slack suites spawn their own uvicorns — a crash or added network timeout there breaks nightly); the chat path must not gain latency (ADR-041's "chat pays zero" was measured, not asserted); the eval harness's cost numbers (ADR-034) must be cross-checkable against traces, which means one price table, not two.

**Decision:** (1) **Langfuse Cloud (free tier) is the recommended backend** for this portfolio project: zero infrastructure to keep alive, a shareable dashboard URL for interviews, and the free tier (50k observations/mo) is an order of magnitude above this project's traffic (~2k observations per full eval run). The local alternative is a real v3 stack — web + worker + Postgres + ClickHouse + Redis + MinIO, six containers for one dev's traces — committed as OPTIONAL `docker-compose.langfuse.yml` (own compose project name, no shared host ports — ClickHouse's default :8123 is the e2e suite's port; headless-init dev keys documented in .env.example) because M6's own verification ran against it and keyless local dev deserves a working path. Not in the main compose file: nothing may depend on it (M1 rule: observability of an optimization is even less of a dependency than the optimization). (2) **One bridge, at the SDK's own seam:** `LangfuseTracingProcessor` implements `agents.tracing.TracingProcessor` and is registered once per process (`init_tracing()` — idempotent; called by `create_app()` and the eval CLI). Every `Runner.run` in the process is traced — router handoffs, specialist spans, tool calls, guardrails, the memory extractor, eval cases — with zero per-callsite work and no second instrumentation layer to drift. Span callbacks build local OTel objects; Langfuse's background thread batches the network. (3) **Empty keys ⇒ verified no-op:** no client is built, no processor registered, `langfuse` never imported (test-pinned). (4) The price table moved to `app/observability/costs.py` — the bridge and `evals/common.py` import the SAME dict (ADR-034's "one committed table" now structurally enforced); generation observations carry `cost_details` computed from it, so Langfuse's native cost aggregation and the harness agree by construction. The MCP server (`mcp_server/`) is NOT traced: it is a separate process that never enters routes_chat and runs plain functions, not agents — there is no SDK trace to bridge, and instrumenting it would be a second mechanism for a surface with no cost and no LLM latency. Documented cut, revisit if MCP traffic ever matters.

**Alternatives:** Langfuse's `openai` drop-in or OTel auto-instrumentation (traces raw LLM calls but not the SDK's agent/handoff/guardrail structure — the handoff-count and per-specialist splits ARE the point); logfire/OpenInference instrumentors (another dependency to version-match against `openai-agents`, and the SDK's processor interface is public API doing exactly this job); tracing_disabled in CI instead of the no-op contract (would silence the SDK's default OpenAI exporter too, changing behavior between CI and prod for no reason); self-hosted Langfuse as the primary (six containers of ops for a portfolio project whose deploy story is one Railway service).

**Tradeoffs:** The bridge touches one private attribute (`observation._otel_span`) to set trace-level OTel attributes — Langfuse v4 has no public setter for a span that isn't the ambient context span; the attribute names themselves (`LangfuseOtelSpanAttributes`) are public API, and the write is version-pinned by tests. Cloud means trace payloads (prompts, tool outputs over seeded fake data) leave the machine — acceptable for synthetic demo data, and the local compose file is the escape hatch if that ever changes.

## ADR-043: The trace/tag schema: population-separating tags at trace end, session as the join key, minimal traces for cache hits, eval cases pinned to their trace ids

Date: 2026-07-08 · Status: accepted

**Context:** Requirement 2-3-5's real problem is aggregation hygiene: the headline numbers (p50/p95 by cache-hit, by specialist; cost per conversation; handoff distribution) silently lie if unlike populations blend. M8 made the traffic genuinely mixed: Slack runs add a sequential injection screen and skip the caches; guardrail trips run NO agent; semantic-cache hits short-circuit BEFORE any agent (no SDK trace at all); the background fact extractor is real spend but not user-facing latency.

**Decision:** Every trace ends with five tags computed by the bridge from the run's own spans — `source:{chat|slack|internal|eval}`, `intent:{first handoff target}`, `agent:{last agent span}`, `cache_hit:{true|false}`, `flagged:{true|false}` (from the guardrail span's triggered bit) — plus metadata the splits need (`input_tokens`, `output_tokens`, `cost_usd`, `handoff_count`). Session identity: `RunConfig.group_id = session_id` becomes the Langfuse session, so "cost per conversation" is a session rollup and the ADR-034 gap closes — e2e/slack flows bill inside the suite-spawned server, invisible to the harness over HTTP, but their traces carry session ids and cost. **Cache hits emit a minimal trace** (routes_chat wraps the short-circuit in `trace("chat")` + one `semantic_cache_hit` custom span, tagged `cache_hit:true`): an ABSENT trace would delete the fast/cheap population from every split and make cached latency unmeasurable — 0 tokens on such traces reports as "unpriced", never $0.00 spent... and never $0.00 as in "free LLM run". The memory extractor runs under its own workflow name (`memory-extraction`) but the SAME session group: excluded from chat latency splits, included in conversation cost — both by construction. Eval cases mint their trace id BEFORE the run (`eval_run_config` — the hook ADR-034 anticipated) and record it in the row JSON; the retrieval-refusal and routing suites (the PR-gate pair) also become Langfuse datasets (`agentdesk-{suite}`, deterministic item ids = sha1(suite+query) so re-runs upsert, one dataset RUN per harness invocation linking each case to its trace). Headline numbers are read back by `scripts/export_metrics.py` (Langfuse trace API + the M3 `/cache/stats` counters BRIDGED as-is, labeled as lifetime numbers) — deliberately a read-only script + Langfuse's own UI, not a fourth Streamlit service.

**Alternatives:** Tagging via `propagate_attributes` context managers at each callsite (only works for the ambient OTel span — the bridge's spans aren't ambient; and N callsites drift, one trace-end aggregator doesn't); no trace on cache hits (see above — the latency split was requirement 3's first line); datasets for all seven suites (e2e/slack/dedup rows aren't single `Runner.run` cases — their evidence is side-effect assertions in the `--out` JSON; linking them buys a UI view of numbers the JSON already has, at real wiring cost); a Streamlit metrics page (a fourth stateless service to keep alive so it can re-implement Langfuse's own dashboards).

**Tradeoffs:** `intent` = first handoff target conflates "router classified X" with direct-specialist eval runs (no handoff ⇒ intent = the agent itself) — fine for the two consumers, documented here. Tags are strings by Langfuse convention (`key:value`), so a renamed agent changes a tag value; the eval floors don't read tags, so nothing gates on them. The export script's cache hit rates come from lifetime Redis counters while its latency/cost numbers come from the query window — two windows, labeled as such in its output rather than papered over.

## ADR-044: The caching A/B: a deliberate CACHES_DISABLED flag distinct from failure, measured on retrieval + e2e, reported per-flow

Date: 2026-07-08 · Status: accepted

**Context:** Requirement 4: produce "caching cut cost X% and latency Y%" honestly. The system already degrades silently when Redis is down — but "simulate an outage" is the wrong OFF arm: it fills logs with warnings, still pays Redis connection timeouts per request, and measures a failure mode rather than a design choice. The comparison also costs real money (LLM re-runs), so the slice matters.

**Decision:** `settings.caches_disabled` (env `CACHES_DISABLED=1`), checked FIRST in all three caches (embedding read+write, semantic lookup+store, response decorator): call straight through, bump NO stats counters (a chosen OFF is not a miss — it must not corrupt the M3 hit-rate history), log nothing (an intentional state is not a warning). The A/B slice is **retrieval + e2e, full suite each, one run per arm**: retrieval is the embedding cache's arena (40 real queries, LLM-free answerable slice so the latency delta is pure embedding-path), e2e is the only suite that goes through routes_chat and therefore the ONLY one that exercises the semantic cache as the product does — including the knowledge_cache flow whose second, paraphrased ask is served `cached=true`. Routing/dedup/quality/graph bypass routes_chat by construction (they call `Runner.run` directly), so running them twice would pay full LLM cost to measure caches they never touch. Reported PER-FLOW (`scripts/ab_caching_report.py` diffs the two `--out` JSONs, `--langfuse-cost` closes the e2e server-side cost gap via each arm's trace window): the semantic cache only fires on paraphrase repeats, so the win concentrates in the knowledge flows — one blended mean would both undersell the win and oversell its breadth. Known and accepted: the OFF arm fails the knowledge_cache flow's `cached=true` assertion — that assertion IS the mechanism under test, and its failure under the flag is the flag working.

**Alternatives:** Simulated Redis outage as the OFF arm (measures degradation, not design — see context); full 7-suite runs twice (~$1.2 and ~100 min to re-measure suites that can't hit the caches); a synthetic paraphrase-hammer benchmark (would show a 99% "win" no real traffic pattern produces; the e2e flows encode the honest repeat rate); flag read at request time via a header (per-request cache toggling invites split-brain measurements; a process-level env flag makes each arm one clean population).

**Tradeoffs:** One run per arm, so arm-to-arm noise (LLM latency variance, ADR-022/028-class flips) rides on the numbers — mitigated by reporting per-flow where the deltas are orders of magnitude, and by quoting the M3 microbenchmark (67-75 ms vs 14-67 s) alongside. `caches_disabled` is read through the lru-cached settings, so it is per-process by design — exactly the granularity the A/B needs, and useless as a runtime kill switch (documented in .env.example so nobody reaches for it in an incident).

## ADR-045: Per-conversation cost budget: Redis-accumulated spend with a loud log + Langfuse WARNING event at the crossing trace

Date: 2026-07-08 · Status: accepted

**Context:** Requirement 6: a configurable per-conversation cost threshold whose crossing is LOUD. "Conversation" is the right unit (a runaway multi-turn session is the failure mode; single turns are bounded by max_turns), but conversations span requests, workers, and restarts — per-trace state isn't enough and in-process accumulators lie under multiple workers.

**Decision:** `settings.cost_alert_threshold_usd` (default $0.10 ≈ 10× a typical multi-turn gpt-5-mini conversation from baseline.json; 0 disables). At each trace end the bridge computes the trace's cost from its generation spans (the ADR-042 table) and accumulates per session in Redis (`cost:session:{id}`, INCRBYFLOAT, 7-day TTL — the M3 stats pattern: persistent, cross-process, and NEVER load-bearing). Crossing the threshold logs `COST BUDGET EXCEEDED` at WARNING with session, cumulative, and trace cost, AND attaches a `cost_budget_exceeded` WARNING event to the trace that crossed — visible exactly where someone debugging the expensive conversation is already looking. Redis down degrades silently to judging each trace alone (a spend metric must never fail a request). Alert-only by design: no throttling, no refusal — a portfolio service desk should page its (imaginary) operator, not deny its user; the HITL gate (ADR-020) is where spend-shaped authority already lives. LLM-free tested: fake spans price a fake conversation over the line.
...
## ADR-047: Reasoning effort is the latency knob — measured span attribution, config-driven `minimal`/`low` defaults, gpt-5-nano router measured and rejected

Date: 2026-07-12 · Status: accepted

**Context:** Cache-miss conversations ran 19–64 s (M6 headline: miss p50 23.6 s). Before touching anything, M10 attributed every millisecond: 6 representative probes × 3 reps through POST /chat against local Langfuse, each trace decomposed into spans (`ignore/tem/m10_latency_baseline.py`/`.json`). Finding: **97–99% of wall time is LLM generation** — all tools together 0.02–0.7 s, handoffs ~1 ms, app overhead <0.2 s. Inside that: the tool-less router's ONE classification call took 6–18 s/conversation, and a Bluetooth-refusal turn billed ~1,800 output tokens for ~60 visible ones — the rest were reasoning tokens at the gpt-5-family default effort (`medium` whenever the request is silent, which this app always was). The knob was verified to actually land before trusting any numbers (the M10 prompt itself mis-remembered the model path as LiteLLM — bare names go to the SDK's default OpenAI Responses provider): the logged `/responses` payload carries `'reasoning': {'effort': …}` and billed reasoning tokens track it exactly (unset→512, minimal→0, low→64, high→1536+ on the same MFA question; `ignore/tem/m10_reasoning_passthrough.py`).

**Decision:** THREE settings in config.py, env-overridable, wired into each agent's `ModelSettings(reasoning=…)` — no instruction, tool, or architecture change. `router_reasoning_effort="minimal"`: single-label classification with forced tool choice needs no deliberation — 0 reasoning tokens, router generation 6–18 s → 1–3 s, full routing suite 30/30 three consecutive runs (accuracy 1.000, hard 6/6, zero ping-pong/integrity; suite wall time itself −62%, 910→342 s). `specialist_reasoning_effort="low"` for **fulfillment + incident**: keeps a thinking budget for order forms and the ADR-021/028 dedup gray band; every gate green at low (dedup 11/12 = historical range, slack 5/5, all e2e order/incident flows); `minimal` not adopted (unproven against the gray bands, savings over low marginal). `knowledge_reasoning_effort="medium"` — **knowledge is deliberately carved out, and this is the milestone's second-most-important finding**: at `low`, the all-suites run failed the e2e stage-2 refusal flow twice in a row (smartwatch probe answered-and-cached). Isolation: direct router-path probes refused 12/12 clean at BOTH efforts; through the REAL chat path (ADR-031 user-facts injected) at `low` the model kept refusing in substance but decorated 5/13 refusals with the forbidden "Sources:" list naming the adjacent phone article — which both violates the ADR-017 output contract and defeats the ADR-023 write-time gate (`is_cacheable` keys on that contract), silently CACHING a refusal. At `medium`: 0 decorated in 6/6 probe runs + every recorded historical e2e run. Mechanism reading: less deliberation → the model pattern-completes its dominant answer format instead of tracking the refusal-branch exception, and only when extra context (injected facts) is in play. Evidence: `ignore/tem/m10_smartwatch_{effort,http}_probe.py` + outputs. Costed honestly: knowledge flows keep only the router saving (answerable −23%, refusal −49%); fulfillment/incident keep the full win (−52…−67% wall). Final 6-probe re-measure: refusal 24.2→12.3 s, knowledge 19.4→15.0 s, order 33.1→11.0 s, incident-link 27.3→13.0 s, multi-intent 64.4→42.5 s; cost/conversation −24…−53% (reasoning bills as output tokens); the PR-gate subset dropped $0.10→$0.07 measured at the final split ($0.056 at all-low). Final-config gates: e2e 18/18, subset PASS (retrieval 1.000 / refusals 10/10 / false refusals 0; routing 10/10). These are BEHAVIOR changes: the defaults are pinned in tests/test_agents.py off the Settings field defaults, and the ADR-026 rule extends to them — changing an effort default needs re-measured eval evidence (the knowledge carve-out is itself the proof the rule has teeth).

**Merge-gate accounting (full `make eval` at router=minimal + all-specialists=low, then re-runs at the final split):** retrieval PASS (recall@5 1.000, refusals 10/10, false refusals 0 — including the smartwatch probe, direct path), routing PASS 30/30, dedup PASS 0.917 vs floor 0.65, quality 4.5/4.7 (report-only), graph PASS. e2e 17/18 ×2 (the smartwatch flow above — root-caused, fixed by the carve-out, re-verified 18/18). Slack first read 2/5 — two of the three failures were a STALE HARNESS ASSERTION, not a regression: the suite asserted the raw ticket uuid in replies, but ADR-046 made `TKTnnn` numbers the reply contract ("confirm by number, never by id"); this run was the first slack-suite execution since that merged. Fixed in `evals/suite_slack.py` (number is the contract, uuid a legacy pass), suite re-run 5/5 — the M5 nightly precedent (fix the probe, never the floor). The third failure was the known ADR-028 gray-band link mode the 0.75 floor tolerates.

**The router-model experiment (measured and rejected):** `TRIAGE_MODEL=gpt-5-nano` at `minimal`, full routing suite twice: accuracy 0.933 then **0.900 — exactly the floor, zero margin** (gpt-5-mini: 1.000, 1.000), with new failure modes: a process question misrouted to fulfillment in BOTH runs, broken-things-to-knowledge misses, ping-pong max 2, one integrity failure. The latency win it was fighting for no longer exists — the mini router at `minimal` is already 1–1.4 s (nano measured 0.8–1.8 s on the same probes, inside noise). Keeping it would also have required an ADR-026 amendment (floors bind to gpt-5-mini) and a costs.py row: found along the way, the price table's prefix matcher would silently price `gpt-5-nano-*` at **gpt-5** rates (`startswith("gpt-5-")`) — `gpt-5-mini` only escapes by dict insertion order; a future nano adoption must insert its row before `"gpt-5"`.

**Alternatives:** Removing the router hop (saves ~1.3 s post-fix vs the 10–35 s this saved; forfeits ADR-003's separation, the routing eval contract, and the ADR-041 guardrail placement — cutting muscle before fat); prompt-shortening/tool trimming (out of scope, instructions pinned); caching refusals (ADR-023 forbids it by design — the fix made refusals cheap instead); streaming (changes perceived latency only — deliberately M11).

**Tradeoffs:** The structural floor remains: ≥2 sequential gpt-5-mini calls ≈ 5–9 s per miss at any effort — stated in the README rather than chased with architecture changes. Lower effort narrows deliberation on genuinely hard turns; the protection is the eval floors (subset per PR, full suites nightly) plus per-env override. M6's committed headline latencies predate this change (README notes it); the next nightly re-baselines.

## ADR-048: Streaming over SSE — POST /chat/stream as a second presentation of the frozen /chat pipeline

Date: 2026-07-12 · Status: accepted

**Context:** M10 left the honest structural floor in place: a cache-miss turn is ≥2 sequential LLM calls ≈ 5–9 s at any reasoning effort, so the remaining lever is *perceived* latency — show the answer as it generates. The hard constraint: the `POST /chat` JSON contract is FROZEN. Its clients are the 18-flow e2e suite, the Slack runner (a pure HTTP client by ADR-038), the eval harness, and the MCP-adjacent surfaces — none may change. Before any endpoint was built, the SDK's streaming behavior was verified live because a bad answer would have gated the design (`ignore/tem/m11_run_streamed_probe.py`): `Runner.run_streamed(...)` with the same `session=` persisted **exactly the same 9 items** to the Postgres store as `Runner.run` on the same query (the follow-up turn's 3,807-token router input proves the history loads; the SDK routes both run styles through the same `save_result_to_session` code), and the ADR-042 `LangfuseTracingProcessor` produced **shape-identical span trees** (chat → task → agent → turn → generation/tool/handoff, same tags, same usage). No degradation ⇒ no design gate.

**Decision:** A NEW endpoint, `POST /chat/stream` — same `ChatRequest` body in, `text/event-stream` out — consumed (for now) only by the Streamlit chat UI; every other client stays on `/chat` (ADR-038's pure-client pattern extended to the browser). SSE over WebSockets because the flow is strictly one-directional (one request, many events: no cancellation or client→server messaging in scope), it rides the existing HTTP stack (same app, same route table — pinned in tests/test_observability.py per the M6 incident — same proxy story), and its failure mode is a dead HTTP response, which is what makes the UI's one-try/except plain-POST fallback possible at all. **Event schema:** `delta` `{text}` (output-text tokens only — reasoning/function-arg deltas are dropped server-side, the browser never sees run internals) · `status` `{stage, detail}` (from run-item events `handoff_occured`/`tool_called`; `transfer_to_*` tool frames suppressed — the SDK models a handoff as a tool call FIRST, and relaying both shows a phantom status) · `final` (the full ChatResponse-equivalent, built from `ChatResponse.model_dump()` so the two payloads cannot drift — parity by construction, pinned by test anyway) · `error`. **The status line is committed at 200 when the stream opens, so mid-stream failures travel in-band as `error` and a stream that ends without `final` IS a failure** — EOF alone is deliberately not a success signal. A semantic-cache HIT emits the stored answer as ONE whole delta + `final` (~0.2 s measured) — no fake token-drip.

**One pipeline, not two:** the invariant-dense steps were extracted from the /chat handler into shared helpers (`_inject_user_facts`, `_serve_cache_hit`, `_run_context`, `_flagged_response`, `_finalize_run` — refactor, not copy) so the endpoints cannot drift; /chat's semantics are unchanged (its 18 e2e flows ran byte-identical datasets and stayed green). Invariants under streaming, each proven: **(a)** ONE `trace("chat")` spans the WHOLE generator, closing when the stream does — live-verified in the TTFT run, trace latency ≈ measured stream wall within 0.02 s on every probe, cache-hit traces present with `cache_hit:true` (the failure mode this kills: a trace closed before the StreamingResponse returns would report ~0 s and unattribute the whole generation); **(b)** the semantic-cache consult/store pair is the same helper with the same first-turn + source gates, and the store runs in `_finalize_run` AFTER `stream_events()` is exhausted — never a partial answer; **(c)** FastAPI's `BackgroundTasks` fires only after a NORMAL response, so extraction rides `StreamingResponse(..., background=...)` — Starlette runs it after the stream closes; the ordering (every streamed event, THEN extraction) is pinned by test; **(d)** streamed runs persist to the Postgres session identically (the live probe above + the e2e parity flow's fresh-session /chat leg); **(e)** guardrail trips surface as a `final` with `flagged=true` — same flag-then-refile contract, never an HTTP error. The UI consumes the stream with `st.write_stream`; the transcript's source of truth is the `final` event (the streamed text is a live preview; the history rerun renders citations/badges exactly as before). Fallback discipline: an `outcome["started"]` flag separates "stream never began" (safe to re-POST /chat) from "stream died mid-run" (do NOT re-submit — the server-side run may have placed an order; show the error copy).

**Measured (same 6 probes as M10, verbatim, 3× each, p50 turn-1; `ignore/tem/m11_ttft_probe.py` / `m11_ttft.json`):** first `status` frame ~1.0–1.2 s on every cache-miss probe (the router's handoff — the first honest sign of life); first *token* 8.8 s (refusal) / 9.2 s (knowledge) / 5.3 s (order) / 9.3 s (incident-link) / 22.3 s (multi-intent turn 1); stream wall ≈ the M10 /chat wall on every probe (10.5 vs 12.3, 13.8 vs 15.0, 10.1 vs 11.0, 10.4 vs 13.0, 34.5 vs 42.5 — differences are run-to-run LLM noise, not a speedup). **The TTFT anatomy is the finding:** text deltas only exist once the FINAL composition call emits visible tokens — everything before is structurally token-free (router's forced handoff, the specialist's tool-call turn, the tool, and the final call's own reasoning prefix, which streams as reasoning deltas we deliberately don't relay). Knowledge runs at `medium` effort (the M10 ADR-047 carve-out that keeps refusals honest), so the milestone's hoped-for ~1 s TTFT is real only for the `status` frame; the first visible token on knowledge paths is seconds by construction. Stated in the README next to what streaming does NOT fix: total wall time and the multi-call structure.

**Disclosed, not fixed (pre-existing, surfaced by the probe):** multi_intent rep 3 was served CACHED — reps 1–2 ended on the knowledge agent (ticket filed mid-run, then the policy answer), so the ADR-023 write gate (`last_agent == knowledge` + contract) stored the answer, and rep 3's *identical* first-turn text hit it, meaning its incident half never acted. This is /chat behavior too, not streaming's: a multi-intent run that writes THEN answers is cacheable, and an exact-text repeat would serve the answer and silently drop the new report. Real users rarely repeat multi-intent messages verbatim (the e2e flow re-words per run and stayed green), but the gate could be tightened to exclude runs containing write-tool calls — deliberately out of M11 scope (requirement: cache logic IDENTICAL) and left as a noted candidate.

**Alternatives:** WebSockets (buys bidirectional messaging — mid-run cancel, typed acks — none needed; adds reconnect/handshake state and a second transport story); content-negotiation on /chat (`Accept: text/event-stream`) — one endpoint changing shape by header is exactly how frozen contracts erode, four client populations would depend on a header they don't send; streaming the Slack/MCP surfaces (their runtimes are message-based — a Slack reply can't render deltas — and ADR-038/040 keep them pure clients of stable contracts).

**Tradeoffs:** SSE over POST forfeits the browser-native `EventSource` auto-reconnect/`Last-Event-ID` resume — acceptable, a broken chat answer is re-asked, not resumed. A client disconnect mid-stream leaves the server-side run finishing (session + trace still complete) — identical to a client abandoning a slow /chat call. The raw ADR-017 "Sources:" footer is briefly visible while streaming (the UI strips it on the history render) — cosmetic, accepted over buffering the stream's tail to hide bookkeeping.
